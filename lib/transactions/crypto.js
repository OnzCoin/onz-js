...
function createTransactionBuffer (transaction, options) {
	...
	function assignTransactionBuffer (transactionBuffer, assetSize, assetBytes) {
		...
		if (transaction.recipientId) {
			var recipient = transaction.recipientId.substring(3);
			var address = '4f4e5a'; //ONZ
			address += bs58check.decode(recipient).toString('hex');
			assignHexToTransactionBytes(transactionBuffer, address);
		} else {
			for (var i = 0; i < 24; i++) {
...
function getId (transaction) {
	var hash = crypto.createHash('sha256').update(getBytes(transaction).toString('hex'), 'hex').digest('hex');
	return hash;
}
